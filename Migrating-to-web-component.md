...
```js
const Decorator =
  NAVSPA.importer < DecoratorPropsV3 > 'internarbeidsflate-decorator-v3';
```
...
useLayoutEffect(() => {
  ...
  const onEnhetChanged = (e: Event) =>
    setEnhet((e as CustomEvent).detail.enhet);
...
```js
document.querySelector('internarbeidsflate-decorator').onBeforeRequest = (
  headers,
) => ({
  ...headers,
